...
class Field:
   ...
   def validate(self, value):
      if value is None and self.required:
         raise ValueError(f'Value {type(self).__name__} is mandatory')

      if value in ('', [], (), {}) and not self.nullable:
...
class EmailField(CharField):
    def validate(self, value):
       super().validate(value)
       if '@' not in value:
          raise ValueError("Value should be valid email")
       return value
...
class PhoneField(Field):
    def validate(self, value):
        ...
        if not str(value)[0] == '7' or not len(str(value)) == 11:
            raise ValueError("Value should start with 7 and has length 11")
        return value
...
class DateField(Field):
    def validate(self, value):
        ...
        try:
            ...
            raise ValueError("Value should be date with DD.MM.YYYY format")
        return value
...
class BirthDayField(Field):
    def validate(self, value):
        ...
        if datetime.datetime.now() - birthday > datetime.timedelta(
            ...
            raise ValueError(f"Birthday value should be no earlier than {AGE_LIMIT} years ago")
        return value
...
class GenderField(Field):
    def validate(self, value):
        ...
        if not isinstance(value, int) or value not in GENDERS:
            raise ValueError("Value should be integer 0, 1 or 2")
        return value
    ...
    def validate(self, value):
        ...
        for item in value:
            if not isinstance(item, int):
                raise ValueError("Value should be list of integers")
        return value
...
def ClientsInterestsHandler(request, ctx, store):
    try:
...
def OnlineScoreHandler(request, ctx, store):
    if request.is_admin:
        score = 42
...
def method_handler(request, ctx, store):
    ...
    method = {'clients_interests': ClientsInterestsHandler,
    ...
    response, code = method[req.method](req, ctx, store)
